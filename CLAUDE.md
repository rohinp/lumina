...
| **M20: Semi-join and Anti-join** | Complete | `JoinType.Semi` / `JoinType.Anti` added to the join enum; `DataFrame.semiJoin()` / `antiJoin()`; LocalBackend uses `exists`/`!exists` over right rows; DuckDB translates to correlated `EXISTS` / `NOT EXISTS` subqueries; join key columns must be distinctly named (LocalBackend merges rows by name, so same-named columns on both sides would collide — same constraint as Inner/Left/Right/Full joins) |
| **M22: Hash Functions** | Complete | `Md5(expr)` → 32-char lowercase hex; `Sha256(expr)` → 64-char lowercase hex; both return null for null input; LocalBackend uses `java.security.MessageDigest` over UTF-8 bytes; DuckDB uses `md5(CAST(... AS VARCHAR))` / `sha256(...)`; digest values are byte-for-byte identical across both backends |
| **M21: First, Last, Median aggregations** | Complete | `First(col)` returns first non-null in group input order; `Last(col)` returns last non-null; `Median(col)` returns middle value (average of two middles for even count), null for empty/all-null groups; all implemented in `LocalBackend` + `DuckDBBackend`; DuckDB `First`/`Last` tests use single-element groups to avoid non-determinism (no ORDER BY clause in aggregate); Java/Kotlin factories added |
| **M23: DataFrame API Ergonomics** | Complete | `where(cond)` — alias for `filter`; `select(String*)` — select columns by name (compiles to `ColumnRef` exprs); `orderBy(col, ascending)` — sort by column name (compiles to `SortExpr`); `col(name)` — instance method shorthand for `ColumnRef`; `Lumina.col(name)` — companion object shorthand; `transform(f)` — applies `f(this)` for composable pipelines; `DataFrameErgonomicsSpec` lives in `lumina-backend-local` test scope (needs `LocalBackend` to run plans) |
