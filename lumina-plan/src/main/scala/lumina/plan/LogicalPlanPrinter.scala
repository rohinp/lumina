...
object LogicalPlanPrinter:
  ...
  private[plan] def exprStr(expr: Expression): String = expr match
    ...
    case Replace(e, s, r)            => s"REPLACE(${exprStr(e)}, '$s', '$r')"
    case RegexpExtract(e, p, g)      => s"REGEXP_EXTRACT(${exprStr(e)}, '$p', $g)"
    case RegexpReplace(e, p, r)      => s"REGEXP_REPLACE(${exprStr(e)}, '$p', '$r')"
    case StartsWith(e, p)            => s"STARTS_WITH(${exprStr(e)}, '$p')"
    case EndsWith(e, s)              => s"ENDS_WITH(${exprStr(e)}, '$s')"
    case LPad(e, l, p)               => s"LPAD(${exprStr(e)}, $l, '$p')"
    case RPad(e, l, p)               => s"RPAD(${exprStr(e)}, $l, '$p')"
    case Repeat(e, n)                => s"REPEAT(${exprStr(e)}, $n)"
    case Reverse(e)                  => s"REVERSE(${exprStr(e)})"
    case InitCap(e)                  => s"INITCAP(${exprStr(e)})"
